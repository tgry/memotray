...
memoManager.createTitle = function(memoId, title, time) {
	...
	t.append($("<input>", {"addClass": "memo_title_label_input", "type": "text", "placeholder": "タイトル"}).val(title).on("change", memoManager.onChangeValue(memoId)));
...
memoManager.createText = function(memoId, text) {
	...
	m.append($("<textarea></textarea>", {"addClass": "memo_text_input", "rows": "" + row, "placeholder": "メモ"}).val(text).on("change", memoManager.onChangeValue(memoId)));
